// video_chat_advanced.cpp - Gelişmiş Karşılıklı Görüntülü İletişim
// Farklı WiFi ağlarında çalışır, yüksek kalite ve FPS korunur
#include <gst/gst.h>
#include <iostream>
#include <string>
#include <thread>
...
private:
    ...
    int framerate = 30;  // 30 FPS (kamera destekli)
    int bitrate = 4000;  // 4 Mbps yüksek kalite
    std::string encoding_preset = "ultrafast";
    bool enable_hardware_acceleration = true;

public:
    AdvancedVideoChat(const std::string& local_ip = "0.0.0.0", int local_port = 5000,
    ...
    bool initialize() {
        ...
        // Yüksek performanslı gönderici pipeline
        std::string send_pipeline_desc = 
            "v4l2src device=/dev/video0 ! "
            "image/jpeg,width=" + std::to_string(video_width) + 
            ",height=" + std::to_string(video_height) + 
            ",framerate=" + std::to_string(framerate) + "/1 ! "
            "jpegdec ! "
            "videoconvert ! "
            "x264enc tune=zerolatency speed-preset=" + encoding_preset + 
        ...
        std::cout << "=== Gelişmiş Video Chat Başlatıldı ===" << std::endl;
    ...
    bool start() {
        ...
        // Heartbeat thread'i başlat (bağlantı kontrolü)
        heartbeat_thread = std::thread([this]() {
            while (is_running) {
                // UDP heartbeat paketi gönder
                send_heartbeat();
    ...
    void set_quality(int width, int height, int fps, int bitrate_kbps) {
        video_width = width;
        video_height = height;
        framerate = fps;
        bitrate = bitrate_kbps;
        std::cout << "Kalite ayarları güncellendi: " << width << "x" << height 
...
AdvancedVideoChat* g_video_chat = nullptr;
...
int main(int argc, char *argv[]) {
    std::cout << "=== Gelişmiş Video Chat - Farklı WiFi Ağları ===" << std::endl;
    std::cout << "Yüksek Kalite: 720p @ 60fps, 4 Mbps" << std::endl;
    std::cout << "Protokol: UDP/RTP" << std::endl;
    ...
    AdvancedVideoChat chat(local_ip, local_port, remote_ip, remote_port);
    g_video_chat = &chat;
    
    // Yüksek kalite ayarları
    chat.set_quality(1280, 720, 60, 4000);  // 720p @ 60fps, 4 Mbps
    ...
    std::cout << "\n=== Gelişmiş Video Chat Aktif ===" << std::endl;
